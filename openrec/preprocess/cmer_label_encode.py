...
def motion_blur(img: np.ndarray, radius: int, sigma: float, angle: float) -> np.ndarray:
...
class Snow(alb.ImageOnlyTransform):
    ...
    def apply(self, img, **params):
        ...
        snow_layer = self.rng.normal(loc=c[0], scale=c[1], size=img.shape[:2])
        snow_layer[snow_layer < c[3]] = 0
        snow_layer = np.clip(snow_layer, 0, 1).astype(np.float32)
        angle = float(self.rng.uniform(-135, -45))
        snow_layer = motion_blur(snow_layer, radius=c[4], sigma=c[5], angle=angle)
        snow_layer = snow_layer[..., None]
        img = c[6] * img
        gray_img = (1 - c[6]) * np.maximum(img,
                                           cv2.cvtColor(img, cv2.COLOR_RGB2GRAY).reshape(h, w, 1) * 1.5 + 0.5)
        img += gray_img
...
class CMERImageProcessor(BaseImageProcessor):
    model_input_names = ["pixel_values", "orig_spatial_shape", "expanded_from_indices", "is_original_flags"]
    def __init__(
        self,
    ...
    def _build_augmentations(self):
        ...
        tlist.extend([
            A.ShiftScaleRotate(
                shift_limit=0, scale_limit=(-.15, 0), rotate_limit=1, border_mode=0,
                interpolation=3, value=[255, 255, 255], p=1
            ),
            A.GridDistortion(
                distort_limit=0.1, border_mode=0, interpolation=3, value=[255, 255, 255], p=0.5
            ),
    ...
    @staticmethod
    def _constant_border(img: np.ndarray, pad_px: int, value: int = 255) -> np.ndarray:
        if pad_px <= 0:
            return img
        if cv2 is not None:
            return cv2.copyMakeBorder(img, pad_px, pad_px, pad_px, pad_px, cv2.BORDER_CONSTANT, value=[value, value, value])
        return np.pad(img, ((pad_px, pad_px), (pad_px, pad_px), (0, 0)), constant_values=value)
    def _maybe_augment_uint8(self, img_uint8: np.ndarray, seed: Optional[int], pre_pad_px: int) -> np.ndarray:
    ...
    def _prep_uint8(self, img, input_data_format) -> np.ndarray:
        ...
        if _fmt == ChannelDimension.FIRST:
            ...
            np_img = to_channel_dimension_format(np_img, ChannelDimension.LAST, input_channel_dim=_fmt)
        ...
        return np_img
    def _resize_uint8(self, img_uint8: np.ndarray, th: int, tw: int, backend: str) -> np.ndarray:
        if backend == "cv2" and cv2 is not None:
            return cv2.resize(img_uint8, (tw, th), interpolation=1)
        return resize(img_uint8, size=(th, tw), resample=self.resample, input_data_format=ChannelDimension.LAST)
    def preprocess_auto(self, images: ImageInput, return_tensors: Optional[Union[str, TensorType]] = None,
                        trainer=None, **kwargs) -> BatchFeature:
    ...
    def preprocess(
        ...
        down_sample_ratio = self.down_sample_ratio if down_sample_ratio is None else int(down_sample_ratio)
        ...
        center_pad = self.center_pad if center_pad is None else bool(center_pad)
        do_augment = self.default_do_augment if do_augment is None else bool(do_augment)
        pre_pad_expand_ratio = self.pre_pad_expand_ratio if pre_pad_expand_ratio is None else float(pre_pad_expand_ratio)
        pre_pad_min_px = self.pre_pad_min_px if pre_pad_min_px is None else int(pre_pad_min_px)
        aug_repeats = self.aug_repeats if aug_repeats is None else max(int(aug_repeats), 0)
        keep_original = self.keep_original if keep_original is None else bool(keep_original)
        num_workers = self.num_workers if num_workers is None else max(int(num_workers), 0)
        pad_num_workers = self.pad_num_workers if pad_num_workers is None else max(int(pad_num_workers), 0)
        resize_backend = (self.resize_backend if resize_backend is None else resize_backend).lower()
        normalize_inplace = self.normalize_inplace if normalize_inplace is None else bool(normalize_inplace)
        images = self.fetch_images(images)
        images = make_flat_list_of_images(images)
        if not valid_images(images):
            raise ValueError("Invalid image type. Must be PIL.Image.Image, numpy.ndarray, or torch.Tensor")
        validate_preprocess_arguments(
        ...
        def _process_one(idx_img: int):
            ...
            if do_augment and self._augmentations is not None and aug_repeats > 0:
                est_pad = max(int(max(h0, w0) * pre_pad_expand_ratio), pre_pad_min_px if pre_pad_expand_ratio > 0 else 0)
                for k in range(aug_repeats):
                    seed_k = None if augment_seed is None else (int(augment_seed) + idx_img * (aug_repeats + 1) + k)
                    aug_img = self._maybe_augment_uint8(base, seed=seed_k, pre_pad_px=est_pad)
                    cand.append((aug_img, False))
            be = ("cv2" if (resize_backend == "auto" and cv2 is not None) else resize_backend)
            max_long_edge = 1024
            for uint8_img, is_orig in cand:
                ...
                if max(hh, ww) > max_long_edge:
                    ...
                    uint8_img = self._resize_uint8(uint8_img, targ_h, targ_w,
                                                ("cv2" if (resize_backend == "auto" and cv2 is not None) else resize_backend))
                    hh, ww = uint8_img.shape[:2]
                MIN_HW = 224
                ...
                if max(ceil_h, ceil_w) <= max_long_edge:
                    ...
                    if floor_h <= 0 or floor_w <= 0:
                        floor_h = max(MIN_HW, min(hh, max_long_edge) // ds * ds)
                        floor_w = max(MIN_HW, min(ww, max_long_edge) // ds * ds)
                    th, tw = floor_h, floor_w
                be = ("cv2" if (resize_backend == "auto" and cv2 is not None) else resize_backend)
                rs_img = self._resize_uint8(uint8_img, th, tw, be)
                if do_rescale:
                    rs_img = rs_img.astype(np.float32)
                    np.multiply(rs_img, float(rescale_factor), out=rs_img, casting='unsafe')
                else:
                    rs_img = rs_img.astype(np.float32)
        ...
        if num_workers and num_workers > 1 and len(images) > 1:
            ...
            for i in range(len(images)):
                ...
                is_orig_flags.extend(flag_i)
        if len(proc_list) == 0:
            return BatchFeature(data={"pixel_values": [], "orig_spatial_shape": [],
                                      "expanded_from_indices": [], "is_original_flags": []},
        ...
        def _maybe_scale_stats_to_image_domain(_arr: np.ndarray, exemplar: np.ndarray) -> np.ndarray:
        ...
        def _make_pad_color(c: int, exemplar: np.ndarray) -> np.ndarray:
            ...
            if pad_value_strategy == "mean":
                ...
                col = np.ones((c,), dtype=np.float32) * (1.0 if do_rescale else 255.0)
                ...
                if pad_value is None:
                    ...
                    if col.shape[0] != c:
                        raise ValueError(f"pad_value length must match channels={c}")
            else:
                col = _mean
            return col
        ...
        def _pad_one(i: int):
            ...
            batched[i] = _to_ch_first(pad_img) if output_channel_format == ChannelDimension.FIRST else pad_img
...
AutoImageProcessor.register("CMER", slow_image_processor_class=CMERImageProcessor)
...
class CMERProcessor(ProcessorMixin):
    ...
    def __call__(
        ...
        **img_kwargs,
        ...
        if ids is None: ids_list = [None] * len(text_list)
        else: ids_list = list(ids)
            
        if categorys is None: cats_list = [None] * len(text_list)
        else: cats_list = list(categorys)
        ...
        try:
            ...
            expanded_ids = [ids_list[src] for src in expanded_from] if ids is not None else None
            expanded_cats = [cats_list[src] for src in expanded_from] if categorys is not None else None
        except IndexError:
             raise ValueError("Index out of range in expanded_from_indices")
        ...
        if expanded_ids: bf["ids"] = expanded_ids
        if expanded_cats: bf["categorys"] = expanded_cats
