...
class SP_TransformerNetwork(nn.Layer):
    ...
    def forward(self, batch_I, weights, offsets, lambda_color=None):
        ...
        batch_weight_params = paddle.unsqueeze(
            paddle.unsqueeze(weights, -1), -1)
        batch_I_power = paddle.stack(
            [batch_I.pow(p) for p in self.power_list], axis=1)

        batch_weight_sum = paddle.sum(batch_I_power * batch_weight_params,
                                      axis=1)
    ...
    def __init__(self,
        ...
        if norm_type == 'BN':
            norm_layer = functools.partial(
                nn.BatchNorm2D, use_global_stats=True)
        elif norm_type == 'IN':
            norm_layer = functools.partial(
        ...
        if self.spt:
            ...
            self.spt_convnet = nn.Sequential(
                # 32*100
                nn.Conv2D(
                    in_channels, 32, 3, 1, 1, bias_attr=False),
                norm_layer(32),
                nn.ReLU(),
                nn.MaxPool2D(
            ...
            self.stucture_fc1 = nn.Sequential(
                nn.Conv2D(
                    128, 256, 3, 1, 1, bias_attr=False),
                norm_layer(256),
                nn.ReLU(),
                nn.MaxPool2D(
                    kernel_size=2, stride=2),
                nn.Conv2D(
                    256, 256, 3, 1, 1, bias_attr=False),
                norm_layer(256),
                nn.ReLU(),  # 2*6
                nn.MaxPool2D(
                    kernel_size=2, stride=2),
                nn.Conv2D(
                    256, 512, 3, 1, 1, bias_attr=False),
                norm_layer(512),
                nn.ReLU(),  # 1*3
                nn.AdaptiveAvgPool2D(1),
                nn.Flatten(1, -1),  # batch_size x 512
                nn.Linear(
                    512, 256, weight_attr=nn.initializer.Normal(0.001)),
                nn.BatchNorm1D(256),
                nn.ReLU())
    ...
    def forward(self, x, return_weight=False):
        ...
        if self.spt:
            ...
            if self.offsets:  # SPIN w. AIN
                ...
                if self.stn:
                    batch_C_prime = sp_weight_fusion[:, self.
                                                     spt_length:, :].reshape([
                                                         x.shape[0], self.F, 2
                                                     ])
                    build_P_prime = self.GridGenerator(batch_C_prime,
            ...
            if self.stn:
                ...
                if build_P_prime_reshape.dtype != paddle.float32:
                    ...
                    build_P_prime_reshape = build_P_prime_reshape.cast(
                        paddle.float32)
                    is_fp16 = True
                x = F.grid_sample(
