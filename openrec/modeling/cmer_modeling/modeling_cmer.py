...
from transformers import PreTrainedModel, PretrainedConfig, GenerationConfig, GenerationMixin, AutoConfig, AutoModel
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        if stride != 1 or in_channels != out_channels:
            self.short = nn.Sequential(nn.Conv2d(in_channels, out_channels, 1, stride), nn.BatchNorm2d(out_channels))
    def forward(self, x):
...
class RoPEMHA(nn.Module):
    def __init__(self, dim: int, num_heads: int, attn_drop: float = 0.1, proj_drop: float = 0.0):
        super().__init__()
...
class PreNormDecoderLayer(nn.Module):
    def __init__(self, hidden_dim: int, num_heads: int, attn_drop_rate: float = 0.1, ffn_ratio: float = 4.0):
        super().__init__()
        self.norm1 = RMSNorm(hidden_dim, eps=1e-6)
        self.mha = RoPEMHA(hidden_dim, num_heads, attn_drop=attn_drop_rate, proj_drop=attn_drop_rate)
...
class CMEREncoder(nn.Module):
    def __init__(self, num_layers: int, num_heads: int, hidden_dim: int, *,
                 down_sample_ratio: int = 16, rope_base: float = 10000.0, gradient_checkpointing: bool = False):
        super().__init__()
        ...
        self.residual_blocks = nn.ModuleList([
            ...
            ResidualBlock(channels[5], channels[6], stride=2 if down_sample_ratio > 16 else 1),
            ResidualBlock(channels[6], channels[7], stride=2),
        ])
        self.fc = nn.Linear(channels[-1], hidden_dim)
        self.vit = nn.ModuleList([
            PreNormDecoderLayer(hidden_dim, num_heads) for _ in range(num_layers)
        ])
    ...
    def _build_rope2d_cache(self, H: int, W: int, device, dtype):
        ...
        if key in self.rope_cache:
            ...
            half = head_dim // 2
            inv_freq = 1.0 / (self.rope_base ** (torch.arange(0, half, 2, device='cpu', dtype=torch.float32) / half))
            ...
            cos_y = cos_y_1d[:, None, :].expand(H, W, half).reshape(H * W, half)
            sin_y = sin_y_1d[:, None, :].expand(H, W, half).reshape(H * W, half)
            cos_x = cos_x_1d[None, :, :].expand(H, W, half).reshape(H * W, half)
            sin_x = sin_x_1d[None, :, :].expand(H, W, half).reshape(H * W, half)
            entry = tuple(t.to(torch.float16).pin_memory() for t in (cos_y, sin_y, cos_x, sin_x))
        ...
        if self.training and torch.is_grad_enabled():
            cos_y = cos_y.clone(); sin_y = sin_y.clone()
            cos_x = cos_x.clone(); sin_x = sin_x.clone()
    ...
    def forward(self, pixel_values: torch.Tensor):
        ...
        if self.gradient_checkpointing and self.training and torch.is_grad_enabled():
            ...
            for layer in self.vit:
                seq = checkpoint(_run_layer, layer, seq, cos_sin_cache, use_reentrant=False)
        else:
            for layer in self.vit:
...
from transformers.models.mbart.modeling_mbart import MBartDecoder
from transformers.modeling_outputs import Seq2SeqLMOutput, BaseModelOutput,CausalLMOutputWithCrossAttentions
import torch.nn.functional as F
import time
import math
...
class CMER(PreTrainedModel,GenerationMixin):
    ...
    def __init__(self, config: CMERConfig):
        ...
        self.lm_head = torch.nn.Linear(decoder_config.d_model, decoder_config.vocab_size, bias=False)
        setattr(self.config, "tie_word_embeddings", True)
        self.tie_weights()
        setattr(self.lm_head, "_dynamic_tied_weights_keys", ["weight"])
        setattr(self.llm_model.embed_tokens, "_dynamic_tied_weights_keys", ["weight"])
    ...
    def load_state_dict(self, state_dict, strict=True):
        if "llm_model.embed_tokens.weight" not in state_dict and "lm_head.weight" in state_dict:
            state_dict["llm_model.embed_tokens.weight"] = state_dict["lm_head.weight"]
        if "lm_head.weight" not in state_dict and "llm_model.embed_tokens.weight" in state_dict:
            state_dict["lm_head.weight"] = state_dict["llm_model.embed_tokens.weight"]
    ...
    def _ensure_swin_safe(self, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        if min(H, W) < need_min:
            ...
            pixel_values = F.interpolate(pixel_values, size=(new_h, new_w),
        ...
        **kwargs,
        ...
        if encoder_outputs is None:
            if pixel_values is None:
                raise ValueError("`pixel_values` must be provided when `encoder_outputs` is not.")
        ...
        if labels is not None:
            ...
            loss = F.cross_entropy(
                shift_logits.view(-1, self.config.decoder_config["vocab_size"]),
                shift_labels.view(-1),
                ignore_index=-100,
                label_smoothing=eps,
            )
        return CausalLMOutputWithCrossAttentions(
            loss=loss,
            logits=logits,
            past_key_values=None if self.training else decoder_outputs.past_key_values,
            hidden_states=None,
            attentions=None,
    ...
    def generate(
        ...
        **kwargs,
        ...
        if num_beams != 1:
            raise NotImplementedError("当前极简 generate 未实现 beam search（num_beams>1）。")
        device = pixel_values.device if pixel_values is not None else next(self.parameters()).device
        encoder_outputs = kwargs.get("encoder_outputs", None)
        if encoder_outputs is None:
            if pixel_values is None:
                raise ValueError("`pixel_values` is required if `encoder_outputs` is not provided.")
            ...
            if isinstance(encoder_outputs, (tuple, list)):
                ...
                raise ValueError("`encoder_outputs` 格式不正确，缺少 last_hidden_state。")
            encoder_hidden_states = encoder_hidden_states.to(device)
        batch_size = encoder_hidden_states.size(0)

        bos_id = kwargs.get("bos_token_id", None)
        ...
        def _top_k_top_p_filtering(logits, top_k=0, top_p=1.0, min_tokens_to_keep=1):
            ...
            if top_k > 0:
                ...
                logits = torch.where(logits < min_thresh, torch.full_like(logits, float("-inf")), logits)
            if top_p < 1.0:
                ...
                logits = torch.zeros_like(logits).scatter(dim=-1, index=sorted_indices, src=sorted_logits)
            return logits
        for _ in range(max_new_tokens):
            ...
            if do_sample:
                ...
                logits = _top_k_top_p_filtering(logits, top_k=top_k, top_p=top_p, min_tokens_to_keep=1)
